...
#转账
def Transfer():
    accountout = int(input("请输入转出账号:"))
    if accountout in bank:
        password = input("请输入转出密码：")
        if password in bank[accountout]["password"]:
            accountin = int(input("请输入转入账号:"))
            if accountin in bank:
                Transferamount = int(input("请输入转账金额:"))
                if Transferamount <= bank[accountout]["money"]:
                    bank[accountout]["money"] -= Transferamount
                    bank[accountin]["money"] += Transferamount
                    print("转账成功！您当前的余额为:",bank[accountout]["money"])
                else:
                    print("您的余额不足！")
            else:
